...
namespace InstallerGenerator
{
	public class EcsInstallerGenerator
	{
		...
		private static string GetBind(TypeElement type)
			=> type.IsDebug
				? $@"			if(isDebug)
				container.BindInterfacesAndSelfTo<{type.Type.Name}>().AsSingle();{(type.Order == 100000 ? "\n" : $"\t// {type.Order:0000} {type.Name}\n")}"
				: $"			container.BindInterfacesAndSelfTo<{type.Type.Name}>().AsSingle();{(type.Order == 100000 ? "" : $"\t// {type.Order:0000} {type.Name}")}\n";
